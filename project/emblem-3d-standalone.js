...
(() => {
  ...
  class SpaceEmblem extends HTMLElement {
    ...
    async _init() {
      ...
      const THREE = await import(window.__resources.threeModule);
      ...
      const map = loader.load(window.__resources.earthMap); map.colorSpace = THREE.SRGBColorSpace; map.anisotropy = 8;
      const night = loader.load(window.__resources.earthNight); night.colorSpace = THREE.SRGBColorSpace;
      const bump = loader.load(window.__resources.earthBump);
      const water = loader.load(window.__resources.earthWater);
