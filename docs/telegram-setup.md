...
אין צורך במפתח Anthropic: ה-webhook רק מכניס את ההודעה לתור, והפירוש נעשה במעבד המקומי על המנוי (ראו `local-runner.md`).
...
צפוי: תשובה מיידית "📥 קיבלתי, מעבד…", ותוך פחות מדקה הודעת "לאשר?" עם שורה `🎉 birthday · דנה כהן · 2026-10-03` וכפתורי ✅/❌. אחרי ✅: "בוצע" והפריט יופיע ב-`/list` ובצג בבלוק "במנהלת השבוע" כשהתאריך בטווח (ברירת מחדל: 10 ימים קדימה).
...
## 6. התראות תקלה (אופציונלי)

כדי לקבל הודעה כשריצה נכשלת או כשנגמרה מכסת המנוי: הוסיפו ל-`.env` של המעבד המקומי את `TELEGRAM_BOT_TOKEN` (אותו token) ואת `TELEGRAM_ALERT_CHAT_ID` (ה-chat id שלכם או של קבוצת ops). אפשר להוסיף את הבוט לקבוצה; ה-chat id של קבוצה מתחיל במינוס.

הפקודה `/status` מחזירה בכל רגע אם המעבד המקומי פעיל, כמה ידיעות ממתינות לעיבוד, כמה הודעות בתור, ואם עבודת המודל בהשהיה בגלל מכסה.
...
| "קיבלתי, אבל המעבד המקומי לא מגיב" | המעבד לא רץ על המחשב | הפעילו `npm run runner`; ההודעה שמורה בתור |
| אין כרטיס אישור אחרי דקה | המעבד עמוס או בהשהיית מכסה | `/status` |
